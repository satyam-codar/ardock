...
import { projectFirestore } from "../gallery/firebase/config";
...
const FacultyImageGrid = ({ setSelectedImg }) => {
  const { docs } = useFirestore("faculty_details");
  ...
  useEffect(() => {
    if (docs) {
      const allCatValue = [
        "all",
        ...new Set(docs.map((categoryName) => categoryName.category)),
      ];
...
export default FacultyImageGrid;
